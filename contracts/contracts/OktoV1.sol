...
import "./OktoManager/IOktoManager.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/utils/Context.sol";
import "@openzeppelin/contracts/token/common/ERC2981.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";

interface IOktoREGISTRY {
    function getWifiDetails(
        uint256 nodeID
    ) external view returns (uint256, address);
}

contract OktoV1 is Context, ERC721Enumerable, ERC2981 {
    /// CONSTANTS
    ...
    IOktoMANAGER OktoRoles;

    IOktoREGISTRY registry;

    modifier whenNotpaused() {
        require(mintPaused == false, "Okto: NFT Minting Paused");
        _;
    }
    ...
    modifier onlyOperator() {
        require(
            OktoRoles.isOperator(_msgSender()),
            "Okto: Unauthorized!"
        );
        _;
    }
    ...
    ///@notice Modifier to ensure that the sender is an operator
    modifier onlyAdmin() {
        require(OktoRoles.isAdmin(_msgSender()), "Okto: Unauthorized!");
        _;
    }
    ...
    constructor(
        ...
        OktoRoles = IOktoMANAGER(OktoManagerContract);
        registry = IOktoREGISTRY(_registryAddr);
    }
    ...
    function updateMetadata(
        ...
        require(
            ...
            "Okto: Non-Existent Token"
        );
        require(
            ownerOf(tokenId) == _msgSender(),
            "Okto: User is not token owner"
        );
    ...
    function setRegistryContract(address registryContract) public onlyAdmin {
        registry = IOktoREGISTRY(registryContract);
    }
    ...
    function mint(string memory metadataURI) external payable {
        require(publicSalePrice >= msg.value, "Okto: Insuffiecient amount!");
        uint256 tokenId = _nextTokenId++;
        _safeMint(_msgSender(), tokenId);
        address payoutAddress = OktoRoles.getPayoutAddress();
    ...
    function requestWifiConnection(uint256 nodeID) external {
        require(balanceOf(_msgSender()) > 0, "Okto: User not authorized");
    ...
    function settleWifiPayment(uint256 duration) external payable {
        require(
            ...
            "Okto: Connection is not accepted"
        );
    ...
    function addFunds() external payable {
        require(msg.value > 0, "Okto: Amount must be greater than zero");
    ...
    function extendVpnValidity(
        ...
        require(duration <= 12, "Okto: More than 12 months not allowed");
        if (OktoRoles.isOperator(_msgSender()) == false) {
            if (fundUse) {
                require(
                    userFunds[_msgSender()] >= msg.value,
                    "Okto: Insufficient funds"
                );
                userFunds[_msgSender()] -= msg.value;
            } else {
                require(
